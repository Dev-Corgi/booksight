...
def updator_bestSeller():
    ...
    try:
        ...
        # 3. 기존 데이터 삭제
        cursor.execute("TRUNCATE TABLE bestseller_list")

        # 4. API 데이터 가져오기
        result = aladin_list_search_api("Bestseller", 1)

        if result is not None:
            # 5. 데이터 삽입
            insert_query = "INSERT INTO bestseller_list (data) VALUES (%s)"
