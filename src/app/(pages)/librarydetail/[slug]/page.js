...
import mapSampleImg from "@png/mapSample.png"
import clockImg from "@png/clock.png"
...
export default function LibraryDetailPage({params}) {
  ...
  useEffect(() => {
    ...
    const result = await getStaticMapHandler({latitude : libraryInfo.libInfo.latitude, longitude : libraryInfo.libInfo.longitude});
  ...
  return (
    ...
    <div className="flex flex-row w-full h-full">
        ...
        <div className="flex flex-row justify-between items-center w-full mt-[40px]">
          ...
          <div className="flex flex-row items-center gap-[6.5px]">
            <div className="relative w-[18px] h-[16px]">
              <Image src={backArrowSVG} alt = "backArrowPic" fill sizes={1}></Image>
            </div>
            <p className="font-NotoSansKRSemiBold text-[16px]">돌아가기</p>
          </div>
        </div>

        <p className = "font-NotoSansKRBold text-[73.73px] w-[497px] h-[154px] leading-[76.9px] mt-[45px] text-pretty">{libraryInfo.libInfo.libName}</p>

       <div className = "flex flex-row gap-[2.37px] mt-[21px]">
        <div className = "relative w-[23px] h-[23px]">
          <Image src = {clockImg} alt = "clockIcon" fill sizes={1}></Image>
        </div>
        <p className = "font-NotoSansKRMedium text-[15.8px]">{libraryInfo.libInfo.operatingTime}</p>
       </div>

        <div className = "flex flex-row gap-[18px] mt-[6px]">
         <div className = "flex flex-row gap-[2.37px]">
          <div className = "relative w-[23px] h-[23px]">
            <Image src = {placeSVG} alt = "placeIcon" fill sizes={1}></Image>
          </div>
          <p className = "font-NotoSansKRMedium text-[15.8px]">{libraryInfo.libInfo.address}</p>
         </div>

         <div className = "flex flex-row gap-[2.37px]">
          <div className = "relative w-[23px] h-[23px]">
            <Image src = {phoneSVG} alt = "phoneIcon" fill sizes={1}></Image>
          </div>
          <p className = "font-NotoSansKRMedium text-[15.8px]">{libraryInfo.libInfo.tel}</p>
         </div>

         <div className = "flex flex-row gap-[2.37px]">
          <div className = "relative w-[23px] h-[23px]">
            <Image src = {linkSVG} alt = "linkIcon" fill sizes={1}></Image>
          </div>
          <Link className = "font-NotoSansKRMedium text-[15.8px]" href={libraryInfo.libInfo.homepage}>홈페이지</Link>
         </div>
        </div>
        ...
        <div className = "flex flex-row h-[100px] mt-[40px]">
          ...
          <div className = "flex flex-row justify-center w-[168.52px] h-[46.34px] ml-[47.18px] mt-[23.09px] rounded-full bg-black gap-[3px]">
            <div className = "relative w-[22px] h-[22px] mt-[11px]">
              <Image src = {connectedImg} alt = "connectedIcon" fill sizes={1}></Image>
            </div>
            <p className = "font-NotoSansKRMedium text-[15px] text-white mt-[10.9px]" href={libraryInfo.libInfo.homepage}>알아보기</p>
          </div>
        ...
        <div className = "flex flex-row gap-[52.66px] mt-[36.76px]">
         <div className = "flex flex-col gap-[5.5px]">
          <p className = "font-MontserratMedium text-[34.76px]">{stockHandler() + "+"}</p>
          <p className = "font-NotoSansKRMedium text-[16px]">도서관 소장 자료</p>
         </div>

         <div className = "flex flex-col gap-[5.5px]">
          <p className = "font-MontserratMedium text-[34.76px]">{weeklyRentHandler() + "+"}</p>
          <p className = "font-NotoSansKRMedium text-[16px]">도서관 주간 대출량</p>
         </div>

         <div className = "flex flex-col gap-[5.5px]">
          <p className = "font-MontserratMedium text-[34.76px]">{dailyRentHandler() + "+"}</p>
          <p className = "font-NotoSansKRMedium text-[16px]">도서관 일간 대출량</p>
         </div>
        </div>

        <div className = "relative w-full h-[262.25px] mt-[40px] rounded-[21px] border-[3px] border-black overflow-clip">
            <Image src={staticMapImg} alt = "mapImage" fill sizes={1} className = "absolute object-cover"></Image>
            <div className = "absolute flex items-center justify-center w-[96px] h-[29px] rounded-full bg-white mt-[9px] ml-[10px]">
              <p className = "font-NotoSansKRMedium text-[13.69px] text-black">오시는 길</p>
            </div>
        </div>
      ...
      <div className="w-[51.74vw] h-screen py-[16px] mr-[16px]">
        <Image src={libraryInfo.picture} alt = "libraryPic" width={1200} height = {1200} className="w-full h-full object-cover rounded-[40px]"></Image>
      </div>
    </div>
    }
    </>
