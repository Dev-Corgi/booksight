...
export async function GET(request) {
  ...
  const Query = searchParams.get('Query') || null; // ISBN number
  const Start = searchParams.get('Start') || 1; 

  const apiUrl = `http://www.aladin.co.kr/ttb/api/ItemSearch.aspx?ttbkey=${authKey}&Query=${Query}&QueryType=Author&SearchTarget=Book&output=js&Version=20131101&MaxResults=50&Start=${Start}`;
